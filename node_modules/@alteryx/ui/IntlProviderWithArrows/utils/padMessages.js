...
var applyPadding = function applyPadding(value, constrainedMultiplier, validatedCharacter, useStar) {
  ...
  var leftPadChar = validatedCharacter || (useStar ? starChar : '⇚');
  var rightPadChar = validatedCharacter || (useStar ? starChar : '⇛');
...
var traverseMessagesObj = function traverseMessagesObj(msgObj, constrainedMultiplier, validatedCharacter, useStar) {
  ...
  if (Array.isArray(msgObj)) {
    return msgObj.map(function (item) {
      return traverseMessagesObj(item, constrainedMultiplier, validatedCharacter, useStar);
    });
  }

  if (isObject(msgObj)) {
    return Object.keys(msgObj).reduce(function (acc, key) {
      return (0, _extends3.default)({}, acc, (0, _defineProperty2.default)({}, key, traverseMessagesObj(msgObj[key], constrainedMultiplier, validatedCharacter, useStar)));
    }, {});
  }
...
var padMessages = function padMessages() {
  ...
  var useStar = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

  if (!isObject(msgObj)) {
    var testingLocaleName = useStar ? 'stars' : 'arrows'; // eslint-disable-next-line no-console

    console.warn("Could not generate ".concat(testingLocaleName, " language. Source messages were either not provided, or were not in the form of an object."));
    return msgObj;
  }
